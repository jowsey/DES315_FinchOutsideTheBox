...
namespace Mirror
{
    ...
    public sealed class NetworkIdentity : MonoBehaviour
    {
        ...
        internal void HandleRemoteCall(byte componentIndex, ushort functionHash, RemoteCallType remoteCallType, NetworkReader reader, NetworkConnectionToClient senderConnection = null)
        {
            ...
            {
                Debug.LogWarning($"NetworkBehaviours is null for {name} [netId={netId}] when receiving {remoteCallType} [{functionHash}]");
                Debug.LogWarning($"Parents are: {string.Join(", ", GetComponentsInParent<Transform>(true).Select(t => t.name + " (active=" + t.gameObject.activeSelf + ")"))}");
                return;
            }
