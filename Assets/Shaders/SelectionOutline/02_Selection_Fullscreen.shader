Shader "02_Selection/Fullscreen"
{
    ...
    float4 FullScreenPass(Varyings varyings) : SV_Target
    {
        ...
        float d = CustomPassLoadCustomDepth(posInput.positionSS);
        ...
        {
            float2 sampleOffset = uvOffsetPerPixel * _OutlineWidth * offsets[i];
            float4 farSample = CustomPassSampleCustomColor(posInput.positionNDC + sampleOffset);

            if (farSample.a > 0.001)
            {
                float4 nearSample = CustomPassSampleCustomColor(posInput.positionNDC + sampleOffset * farSample.a);
                outline = max(nearSample, outline);
            }
        }
